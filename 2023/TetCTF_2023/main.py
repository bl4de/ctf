...
NewYearCategoryList = ["NewYearCommonList",
                       "NewYearHealthList", "NewYearFamilyList", "NewYearWealthList"]
NewYearCommonList = ["Chúc Mừng Năm Mới!", "Happy New Year!", "Cheers to a new year and another chance for us to get it right!",
                     "Let's make 2023 our best year yet.", "Năm mới Vui Vẻ", "New year! New Me!"]
NewYearHealthList = ["Sức Khoẻ Dồi Dào!", "Wishing you and yours health and prosperity in the new year",
                     "May the new year bring you peace", "Joy, and happiness!", "Tết An Lành"]
NewYearFamilyList = ["Gia Đình Hạnh Phúc!", "Hoping that the new year will bring you and your family a year full of joy and serenity",
                     "Together! Forever!", "Best wishes for your family"]
NewYearWealthList = ["Năm Mới Phát Tài!", "Have a sparkling New Year!", "Here's hoping you make the most of 2023!",
                     "May the new year bless you with health, wealth, and happiness.", "An Khang Thịnh Vượng"]
                    ...
                    if greetType != None and greetNumber != None:
                        ...
                        if greetType.isidentifier() == True and botValidator(greetNumber) == True:
                            if len("%s[%s]" % (greetType, greetNumber)) > 20:
                                ...
                                greeting = eval("%s[%s]" %
                                                (greetType, greetNumber))
                            try:
                                if greeting != fail and debug != None:
                                    greeting += "<br>You're choosing %s, it has %s quotes" % (
                                        greetType, len(eval(greetType)))
                            except:
                                pass
                        else:
                            greeting = fail
                    else:
                        greeting = random_greet(
                            random.choice(NewYearCategoryList))
                except:
                    greeting = fail
                    pass
        else:
